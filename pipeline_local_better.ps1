...
$etl_args = @(
    ...
    "--platinum_version", $new_version.platinum_version.dev,
...
# ---------------------------- Features for Model Periodo (New Version) -------------------------
foreach ($tipo in $script_types) {
    ...
    if ($mapping_tipos[$tipo]) {
        ...
        $feats_version = $new_version.feats_version.$tipo.dev
        $target_version = $new_version.target_version.$tipo.dev
...
# ----------------------------- Models for Model Periodo (New Version) -----------------------------
foreach ($tipo in $script_types) {
    $mapping_tipos = Get-MappingTipos -periodo $model_periodo
    if ($mapping_tipos[$tipo]) {
