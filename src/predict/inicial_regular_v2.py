...
class TrainInicial:
    ...
    def get_data_predict(self, periodo:int, model):
        ...
        data_model_eval = pd.DataFrame(data = np.random.randint(0, 2, size=(100, 3)), columns=['PERIODO_TARGET', 'CURSO_ACTUAL', 'HORARIO_ACTUAL'])

        return data_model_eval
    

    def upload_data_predict(self, model_version:str, periodo:int, df_model_predict:pd.DataFrame):
        ...
        path_model_version = self.output_predict_inicial_datastore/"test"/model_version
...
def parse_args():
    ...
    parser.add_argument("--input_model_inicial_path", dest='input_model_inicial_path', 
                        type=str, help="Ruta del modelo (artifact)")
    parser.add_argument("--input_feats_inicial_datastore", dest='input_feats_inicial_datastore',
                        type=str)
    parser.add_argument("--input_target_inicial_datastore", dest='input_target_inicial_datastore',
                        type=str)
    parser.add_argument("--output_predict_inicial_datastore", dest='output_predict_inicial_datastore',
                        type=str)
